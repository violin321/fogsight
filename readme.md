...
- **Browser-based WebM 视频导出 MVP**：支持桌面版 Chrome / Edge，通过 `getDisplayMedia` + `MediaRecorder` 录制预览窗口。导出时需要手动选择 Recording 窗口；停止共享后浏览器会下载 `.webm` 文件。移动端暂不支持视频导出，但仍可保存 HTML 成果。
- **Regenerate**：基于原始 topic 重新生成新版本，不污染当前对话历史，便于回到初始意图重做。
- **Improve this version**：基于当前 HTML 结果继续优化视觉层次、动效、移动端适配和解释清晰度。
- **若干可用性修复**：包括 Open in new window 的滚动修复、fenced code 中 `html` 标记清理，以及更强的响应式 prompt 引导。
...
4. 需要录屏交付时，在桌面版浏览器中使用 **导出视频**，手动选择 Recording 窗口并在完成后停止共享下载 WebM。
...
* **浏览器端视频导出 MVP**: 在桌面 Chrome / Edge 中可通过浏览器录屏链路导出 WebM，适合快速交付预览或演示。  
...
6. **生成后继续操作：**
   ...
   - 需要导出录屏：在桌面 Chrome / Edge 中使用视频导出并手动选择 Recording 窗口。
